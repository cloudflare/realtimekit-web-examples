...
Recording SDK based example application that records only participants with the `LEAD` preset. Uses `RtkSimpleGrid` for camera-only views and `RtkMixedGrid` when a screenshare is active.
